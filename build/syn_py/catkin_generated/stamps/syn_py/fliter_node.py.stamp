...
class Fliter():
    def __init__(self):
        rospy.init_node('fliter_node',anonymous=True)
    ...
    def callback(self, camera_info_msg, zed_camera_info_msg, 
                 ...
                 tf_msg, ouster_imu_msg, zed2_imu_msg):
        ...
        try:
            ...
            image_pan = self.bridge.imgmsg_to_cv2(image_msg, desired_encoding='bgr8')
            image_zed = self.bridge.imgmsg_to_cv2(zed_image_msg, desired_encoding='bgr8')
        ...
        #投影点云到ZED图像平面
        self.point_to_image_proj(transformed_cloud_zed,projected_image_zed)
        #投影点云到全景相机图像平面
        ...
        try:
            ...
            output_msg_zed = self.bridge.cv2_to_imgmsg(projected_image_zed, encoding="bgr8")
        ...
        try:
            # 将OpenCV图像转换为ROS Image消息
            output_msg_pan = self.bridge.cv2_to_imgmsg(projected_image_pan, encoding="bgr8")
            # 保持与原始消息相同的header信息（时间戳+坐标系）
    ...
    def projection_calculator(X,Y,Z,image_width,image_height):
        # 计算水平角度（范围：-π 到 π）
